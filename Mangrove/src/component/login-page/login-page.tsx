...
const LoginPage = ({route}) => {
  ...
  const saveUserToSQLiteLogin = (email, password) => {
    db.transaction(tx => {
      tx.executeSql(
        `INSERT INTO Users (email, password, pin, emailConfirm, name, area, fingerPrint) 
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [email, password, 0, 0, null, null, 0], // Default values: 0 for pin, emailConfirm, fingerPrint and null for name, area
        (tx, result) => {
  ...
  const checkLocalStoredData = () => {
    ...
    db.transaction(tx => {
      tx.executeSql(
        'SELECT emailConfirm FROM Users WHERE email = ?',
        [email],
        (tx, results) => {
          if (results.rows.length > 0) {
            console.log('R');
