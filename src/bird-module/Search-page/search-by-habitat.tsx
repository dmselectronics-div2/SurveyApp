...
const SearchByHabitat = ({setShowHabitatFilter, onEditItem}: {setShowHabitatFilter: (v: boolean) => void; onEditItem?: (item: any) => void}) => {
  ...
  const handleSearch = async () => {
    if (!selectedHabitat) {
      Alert.alert('Select Habitat', 'Please select a habitat type to search.');
      return;
    }
    ...
    try {
      ...
      const filtered = data.filter((item: any) => {
        ...
        const matchHabitat = item.habitatType === selectedHabitat;
        const matchStart = !startD || itemD >= startD;
        const matchEnd = !endD || itemD <= endD;
        return matchHabitat && matchStart && matchEnd;
      });
      setResults(filtered);
    } catch {
      setResults([]);
  ...
  const handleDelete = (item: any, index: number) => {
    Alert.alert('Delete Survey', `Delete survey at ${item.habitatType} on ${item.date}?`, [
  ...
  const downloadCSV = async () => {
    ...
    const path = `${RNFS.ExternalStorageDirectoryPath}/Download/HabitatFilter_${timestamp}.csv`;
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <TouchableOpacity
          onPress={() => setShowHabitatFilter(false)}
          style={styles.backBtn}>
          <IconButton icon="arrow-left" iconColor="#333" size={22} />
        </TouchableOpacity>
        <View style={styles.headerContent}>
          <Text style={styles.headerTitle}>Habitat Filter</Text>
          <Text style={styles.headerSubtitle}>Filter by habitat type</Text>
        </View>
      </View>
        ...
        <View style={styles.card}>
          <View style={styles.cardHeader}>
            <View style={styles.iconContainer}>
              <Icon name="tree-outline" size={22} color={GREEN} />
            </View>
          ...
          <View style={styles.resultsSection}>
            ...
            {results.length === 0 ? (
              <View style={styles.emptyCard}>
                <Icon name="tree-outline" size={48} color="#ccc" />
                <Text style={styles.emptyText}>No records found</Text>
                <Text style={styles.emptySubtext}>Try a different habitat type or date range</Text>
              </View>
            ) : (
                ...
                <View key={item._id} style={styles.resultCard}>
                  <View style={styles.resultHeader}>
                    <View style={styles.resultHeaderLeft}>
                      <View style={styles.habitatBadge}>
                        <Text style={styles.habitatBadgeText}>{item.habitatType}</Text>
                      </View>
                      <View>
                  ...
                  <View style={styles.detailsGrid}>
                    <View style={styles.detailItem}>
                      ...
                      <Text style={styles.detailValue}>{item.point}</Text>
                    </View>
                    <View style={styles.detailItem}>
...
const styles = StyleSheet.create({
  ...
  habitatBadge: {backgroundColor: GREEN, paddingHorizontal: 10, paddingVertical: 6, borderRadius: 6},
  habitatBadgeText: {color: '#fff', fontSize: 11, fontWeight: '700'},
...
export default SearchByHabitat;
