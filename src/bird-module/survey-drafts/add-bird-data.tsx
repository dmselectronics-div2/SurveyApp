...
const data15 = birdSpecies.map(item => {
  const regex = /^(.*)\(([^)]+)\)$/;
  ...
  return {
    ...item,
    customLabel: `${name} (${scientificName})`, // Use plain text for the label
    formattedLabel: (
      <Text>
        {name} (<Text style={{ fontStyle: 'italic' }}>{scientificName}</Text>)
      </Text>
    ),
  };
});
...
const openGallery1 = () => {
  ...
  valueField="value"
  placeholder={
    !isFocus1 && species && species.includes('(')
      ? (() => {
          const parts = species.split('(');
          const common = parts[0].trim();
          const scientific = parts[1]?.split(')')[0] || '';
          return `${common} (${scientific})`;
        })()
      : 'Select Observed Species'
  }
  ...
  renderItem={item => {
    ...
    const commonName = labelParts[0]?.trim() || item?.label || '';
    const scientificName = valueParts[1]?.split(')')[0] || '';
